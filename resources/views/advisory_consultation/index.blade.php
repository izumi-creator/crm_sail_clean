...
@section('content')
    ...
    <div class="w-full p-6"> <!-- ✅ ダッシュボードと統一 -->
        <h2 class="text-2xl font-bold mb-4 text-gray-800">顧問相談一覧</h2>

        <!-- 🔍 検索フォーム -->
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <form method="GET" action="{{ route('advisory_consultation.index') }}">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="advisory_party" class="w-full border px-3 py-2 rounded">
                            <option value="">選択してください</option>
                            @foreach (config('master.advisory_parties') as $key => $label)
                                <option value="{{ $key }}" {{ request('advisory_party') == (string)$key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-4 flex justify-end space-x-2">
                    ...
                    <a href="{{ route('advisory_consultation.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded">検索条件クリア</a>
                    <a href="{{ route('advisory_consultation.create') }}" class="bg-green-500 text-white px-4 py-2 rounded">新規登録</a>
                </div>
            </form>
        </div>
...
<div class="mb-10">
    <h2 class="text-xl font-semibold mb-4">検索結果（{{ $advisory_consultations->total() }}件）</h2>
    <div class="p-4 border rounded-lg shadow bg-white">
        <div class="overflow-y-auto max-h-48"> <!-- ✅ 高さをダッシュボードと統一 -->
            <table class="w-full border-collapse border border-gray-300 table-fixed">
                ...
                <tbody class="text-sm">
                    @foreach ($advisory_consultations as $advisory_consultation)
                    <tr>
        ...
        <div class="mt-8 flex items-center space-x-4">
            <p class="text-gray-600">
                {{ __('pagination.showing', ['first' => $advisory_consultations->firstItem(), 'last' => $advisory_consultations->lastItem(), 'total' => $advisory_consultations->total()]) }}
            </p>
            {{ $advisory_consultations->links('vendor.pagination.custom') }}
        </div>
    </div>
</div>
