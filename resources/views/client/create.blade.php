...
<div class="p-6 border rounded-lg shadow bg-white">
    ...
    <div class="p-6 border border-gray-300 border-t-0 text-sm">
        ...
        <!-- 個人用項目 -->
        <div id="individual-fields" class="grid grid-cols-2 gap-6 hidden">
            ...
            <div class="col-span-2 mt-6 bg-blue-100 text-blue-900 font-semibold py-2 px-6 -mx-6">
                ...
                <div>
                 ...
                 <div>
                     <label class="block font-semibold mb-1">本人確認書1</label>
                     <input type="text" name="individual[identification_document1]" class="w-full p-2 border rounded bg-white">
                     @errorText('individual.identification_document1')
                 </div>
                 <div>
                     <label class="block font-semibold mb-1">本人確認書2</label>
                     <input type="text" name="individual[identification_document2]" class="w-full p-2 border rounded bg-white">
                     @errorText('individual.identification_document2')
                 </div>
                 <div>
                     <label class="block font-semibold mb-1">本人確認書3</label>
                     <input type="text" name="individual[identification_document3]" class="w-full p-2 border rounded bg-white">
                     @errorText('individual.identification_document3')
                 </div>
                 <div></div>
            ...
            <div class="col-span-2 mt-2 -mx-6">
                ...
                <div class="accordion-content hidden pt-4 px-6">
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block font-semibold mb-1">自宅連絡不可区分</label>
                            <select name="individual[not_home_contact]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.not_home_contacts') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('individual.not_home_contact') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div></div>
                        <div>
                            ...
                            <input type="text" name="individual[address_postalcode]" 
                                id="individual_address_postalcode"
                         ...
                         <div>
                            ...
                            <input type="text"name="individual[contact_postalcode]"
                               id="individual_contact_postalcode"
                        ...
                        <div>
                            <label class="block font-semibold mb-1">住所_名（漢字）</label>
                        ...
                        <div>
                            <label class="block font-semibold mb-1">住所（郵送先）_名（漢字）</label>
                        ...
                        <div>
                            <label class="block font-semibold mb-1">住所_名（ふりがな）</label>
                        ...
                        <div>
                            <label class="block font-semibold mb-1">住所（郵送先）_名（ふりがな）</label>
            ...
            <div class="col-span-2 mt-2 -mx-6">
                ...
                <div class="accordion-content hidden pt-4 px-6">
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block font-semibold mb-1">年賀状を送る</label>
                            <select name="individual[send_newyearscard]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_types') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('individual.send_newyearscard') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @errorText('individual.send_newyearscard')
                        </div>                
                        <div>
                            <label class="block font-semibold mb-1">暑中見舞いを送る</label>
                            <select name="individual[send_summergreetingcard]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_types') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('individual.send_summergreetingcard') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @errorText('individual.send_summergreetingcard')
                        </div>
                        <div>
                            <label class="block font-semibold mb-1">事務所報を送る</label>
                            <select name="individual[send_office_news]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_types') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('individual.send_office_news') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @errorText('individual.send_office_news')
                        </div>
                        <div>
                            <label class="block font-semibold mb-1">交際情報履歴を自動作成</label>
                            <select name="individual[send_autocreation]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_autocreations') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('individual.send_autocreation') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
        ...
        <!-- 法人用項目 -->
        <div id="corporate-fields" class="grid grid-cols-2 gap-6 hidden">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="corporate[address_postalcode]"
                                    id="corporate_address_postalcode"
                             ...
                             <div>
                                ...
                                <input type="text" name="corporate[contact_postalcode]"
                                    id="corporate_contact_postalcode"
            ...
            <div class="col-span-2 mt-2 -mx-6">
                ...
                <div class="accordion-content hidden pt-4 px-6">
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <!-- 年賀状を送る send_newyearscard -->
                        <div>
                            <label class="block font-semibold mb-1">年賀状を送る</label>
                            <select name="corporate[send_newyearscard]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_types') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('corporate.send_newyearscard') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @errorText('corporate.send_newyearscard')
                        </div>                
                        <!-- 暑中見舞いを送る send_summergreetingcard -->
                        <div>
                            <label class="block font-semibold mb-1">暑中見舞いを送る</label>
                            <select name="corporate[send_summergreetingcard]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_types') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('corporate.send_summergreetingcard') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @errorText('corporate.send_summergreetingcard')
                        </div>
                        <!-- 事務所報を送る send_office_news -->
                        <div>
                            <label class="block font-semibold mb-1">事務所報を送る</label>
                            <select name="corporate[send_office_news]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_types') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('corporate.send_office_news') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                            @errorText('corporate.send_office_news')
                        </div>
                        <!-- 交際情報履歴を自動作成 send_autocreation -->
                        <div>
                            <label class="block font-semibold mb-1">交際情報履歴を自動作成</label>
                            <select name="corporate[send_autocreation]" class="w-full p-2 border rounded bg-white">
                                @foreach (config('master.send_autocreations') as $key => $label)
                                    <option value="{{ $key }}" @selected(old('corporate.send_autocreation') == $key)>{{ $label }}</option>
                                @endforeach
                            </select>
    ...
    <div id="resetModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        ...
        <script>
            // ページロード時に選択済みのラジオに応じてフィールド表示切り替え
            window.addEventListener('DOMContentLoaded', function () {
                const selected = document.querySelector('input[name="client_type"]:checked');
                if (selected) {
                    toggleClientType(selected.value == '1' ? 'individual' : 'corporate');
                }
                ...
                // 入力不可にする
                if (nameKanji) nameKanji.readOnly = true;
                if (nameKana) nameKana.readOnly = true;
                ...
                // イベントリスナー設定
                [lastNameKanji, firstNameKanji].forEach(el => {
                    el.addEventListener('input', updateFullNameKanji);
                });
            
                [lastNameKana, firstNameKana].forEach(el => {
                    el.addEventListener('input', updateFullNameKana);
                });
            });
        
            // クライアント区分に応じて表示切り替え＆他のラジオを無効化
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                zipInputs.forEach(input => {
                    ...
                    const errorFieldId = input.dataset.zipError; // 任意指定可能（なければ無視）
                    ...
                    input.addEventListener('blur', function () {
                        ...
                        if (!/^\d{7}$/.test(zipcode)) {
                            if (errorField) {
                                errorField.textContent = "7桁の数字で入力してください（例: 123-4567）";
                            }
                            return;
                        } else {
                            if (errorField) {
                        ...
                        fetch(`https://zipcloud.ibsnet.co.jp/api/search?zipcode=${zipcode}`)
                            ...
                            .then(data => {
                                if (data.results && data.results[0]) {
                                    ...
                                    if (errorField) {
                                        const msg = data.message || "該当する住所が見つかりませんでした。";
                            ...
                            .catch(error => {
                                ...
                                if (errorField) {
                                    errorField.textContent = "通信エラーが発生しました。";
                                }
                            });
                    });
                });
