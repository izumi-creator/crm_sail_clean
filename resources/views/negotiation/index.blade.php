...
@section('content')
    ...
    <div class="w-full p-6"> <!-- ✅ ダッシュボードと統一 -->
        <h2 class="text-2xl font-bold mb-4 text-gray-800">折衝履歴</h2>

        <!-- 🔍 検索フォーム -->
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <form method="GET" action="{{ route('negotiation.index') }}">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-gray-700 font-semibold">登録日</label>
                        <input type="date" name="record_date" value="{{ request('record_date') }}" 
                            class="w-full border px-3 py-2 rounded">
                    </div>
                    <div>
                ...
                <div class="mt-4 flex justify-end space-x-2">
                    ...
                    <a href="{{ route('negotiation.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded">検索条件クリア</a>
                    <a href="{{ route('negotiation.create') }}" class="bg-green-500 text-white px-4 py-2 rounded">新規登録</a>
                </div>
            </form>
        </div>
...
<div class="mb-10">
    <h2 class="text-xl font-semibold mb-4">検索結果（{{ $negotiations->total() }}件）</h2>
    <div class="p-4 border rounded-lg shadow bg-white">
        ...
        <div class="mt-8 flex items-center space-x-4">
            <p class="text-gray-600">
                {{ __('pagination.showing', ['first' => $negotiations->firstItem(), 'last' => $negotiations->lastItem(), 'total' => $negotiations->total()]) }}
            </p>
            {{ $negotiations->links('vendor.pagination.custom') }}
        </div>
    </div>
</div>
