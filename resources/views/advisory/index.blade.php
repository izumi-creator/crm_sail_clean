...
@section('content')
    ...
    <div class="w-full p-6"> <!-- ✅ ダッシュボードと統一 -->
        <h2 class="text-2xl font-bold mb-4 text-gray-800">顧問契約一覧</h2>

        <!-- 🔍 検索フォーム -->
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <form method="GET" action="{{ route('advisory.index') }}">
...
<div class="mb-10">
    <h2 class="text-xl font-semibold mb-4">検索結果（{{ $advisories->total() }}件）</h2>
    <div class="p-4 border rounded-lg shadow bg-white">
        ...
        <div class="mt-8 flex items-center space-x-4">
            <p class="text-gray-600">
                {{ __('pagination.showing', ['first' => $advisories->firstItem(), 'last' => $advisories->lastItem(), 'total' => $advisories->total()]) }}
            </p>
            {{ $advisories->links('vendor.pagination.custom') }}
        </div>
    </div>
</div>
